...
import {OmnisharpClient, Context, DriverState} from "./omnisharp-client"
import {CompositeObservable, ICompositeObservable} from "./rx/CompositeObservable";


export class CompositeOmnisharpClient implements OmniSharp.Events {
    static clientKeys = _.keys(OmnisharpClient.prototype);
    private _keys: string[];
    private _clients: OmnisharpClient[] = [];

    constructor(clients: OmnisharpClient[]) {
        for (var client of clients) {
            this.addObserversForClient(client);
        }
    }

    public add(client: OmnisharpClient) {
        this.addObserversForClient(client);
    }

    public remove(client: OmnisharpClient) {
        this.removeObserversForClient(client);
    }

    private setupKeys(client: OmnisharpClient) {
        _.each(client, (value: any, key) => {
            if (value.subscribe && _.contains(CompositeOmnisharpClient.clientKeys, key)) { // assume its an observable
                this._keys.push(key)
    ...
    private addObserversForClient(client: OmnisharpClient) {
    ...
    private removeObserversForClient(client: OmnisharpClient) {
        if (!this._keys || !this._keys.length) { this.setupKeys(client); }
