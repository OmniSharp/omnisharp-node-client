...
gulp.task('sync-clients', [], function() {
    ...
    _.each(['v1', 'v2'], function(version) {
        ...
        var regex = new RegExp('export module OmniSharp\\.Events {[\\s\\S]*?interface ' + VERSION + ' {([\\s\\S]*?)}');
        ...
        var regex2 = new RegExp('export module OmniSharp\\.Events\\.Aggregate {[\\s\\S]*?interface ' + VERSION + ' {([\\s\\S]*?)}');
...
gulp.task('npm-prepublish', ['typescript', 'babel'/*, 'omnisharp-client-declaration'*/]);
